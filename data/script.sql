...
SELECT d.*, cl.*, SUM(p.montantpay) AS verse, (d.montantd - SUM(p.montantpay)) AS restant 
...
SELECT d.idd, cl.nom, cl.prenom, cl.email, cl.telephone, d.numero, d.dated, d.montantd AS montantd, 
    ...
    GROUP BY d.idd, cl.idcl HAVING restant > 0 AND cl.telephone = '763048794';

/*une requette qui filtre par date*/
SELECT d.idd, cl.nom, cl.prenom, cl.email, cl.telephone, d.numero, d.dated, d.montantd AS montantd, 
    SUM(p.montantpay) AS verse, (d.montantd - SUM(p.montantpay)) AS restant 
    FROM dette d JOIN client cl ON d.idcl = cl.idcl LEFT JOIN paiement p ON d.idd = p.idd 
    GROUP BY d.idd, cl.idcl HAVING restant > 0 AND d.dated = '2024-07-20';
...
FROM 
    client cl
JOIN 
    dette d ON cl.idcl = d.idcl
LEFT JOIN 
    artDette ad ON d.idd = ad.idd
LEFT JOIN 
    article a ON ad.idA = a.idA
LEFT JOIN 
    paiement p ON d.idd = p.idd
WHERE 
    cl.idcl = 1;
